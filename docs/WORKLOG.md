...
12. **引号内拟声词漏网修复**（commit 后推，后端重新部署）：「咯嘣咯嘣」包在直角引号里被引号救援拆成独立段后，进入分析缓存时仍带说话人 role——在 `analyze_long` 缓存前再跑一次拟声词兜底（`assign_speakers` 处的兜底保持双保险）。用《西游记》偷金丹原文线上实测：叙述+拟声=旁白、「师父，吃一个。」=悟空 ✓。
13. **迷你播放栏上线**（commit 后推，story.html 已部署）：列表页底部常驻显示当前播放篇目（封面+标题+状态+播放/暂停+细条进度），与播放页/工作台风格一致。状态机用 `sqNowPlaying` localStorage 持久化（saveProgress 每 5s 同步）— 退出/重进/换设备都能从工作台跳回时恢复上次的篇目+进度+已暂停状态。点击迷你栏跳播放页保持进度；播放页打开时迷你栏自动隐藏；听完/切换篇目自动覆盖。390px 移动断点收紧。重构 renderMini 不再控制 display（避免 setNowPlaying 与 showMiniIfListVisible 时序冲突）。
14. **迷你栏恢复重播开场修复**（commit 后推，已部署）：恢复路径两个坑——①开场片段播放期间退出时 sq_prog 不落盘，恢复被当全新开始重播开场（openNowPlaying 现传 `_miniResumeMs`，优先级高于 sq_prog，直接 startStoryAt 续播正文）；②新 Audio 元素 metadata 未就绪时设置 currentTime 被静默忽略，从 0 开始播（startStoryAt 延迟到 loadedmetadata 再 seek）。实测：posMs=45.2s 恢复后 curClip=null、位置连续（144.6→198s 跨会话无缝续播）。
15. **卡顿修复 + 恢复位置覆盖 bug**（commit 后推，已部署）：①pagehide 在"本页无音频实例"时也强制落盘 nowPlaying，陈旧内存值覆盖新存档（恢复位置总被踩回旧的）——改为仅 curAudioId 匹配存活音频才写；②setNowPlaying 重置 posMs=0 导致 openNowPlaying 兜底 seek 被跳过——恢复位置经 setNowPlaying 参数贯穿；③性能：saveNowPlaying 节流 3s（force 旁路）、迷你进度 <0.4% 不写 DOM、装饰层/迷你栏加 will-change、backdrop blur 14→10px。受控验证：init 45200 → 点播 → 46.2s 续播、无开场、不跳页 ✓。
16. **跨端共享书架上线**（commit 8858991，前后端已部署）：PG 新表 user_audiobooks（migration 20260831131000，音频 base64 存 PG，上限 45MB）；后端 4 接口——POST /me/audiobooks（从 done 任务保存）、GET 列表、GET <id>/audio（Range 206 流式、uuid 不鉴权）、DELETE。工作台主区加「创作/书架」视图切换，生成完成后「💾 保存到书架」（标题可改、登录门槛）；睡前故事书架移除上传/有声化入口，仅收听工作台保存的成品（进度走同域 sq_prog_* 自动一致）。边界：空书架文案、未登录引导、保存中防重、删除清进度、数据异常降级提示。E2E：真实任务保存→列表→Range 206→删除→空列表 ✓；生产 UI 双端验证 ✓（收听按钮的自动化断言因测试环境冻结未完成，需人工点一次确认）。
17. **工作台保存 ReferenceError 修复 + 视图 tab 图标升级**（commit 8e84b5e 后推，已部署）：index.html 书架保存流程误调了 story.html 的 toast/showPvModal（6 处 ReferenceError），全部替换为 index.html 已有提示（noticeBox / confirm / alert）。将「✍️ 创作」「📚 书架」emoji 换成内联细线 SVG（羽毛笔 + 书堆），stroke=currentColor 继承活跃态琥珀色，与衬线暖纸风格统一。
18. **保存按钮 + 书架标题视觉重塑**（commit 8a46dce 后推，已部署）：保存按钮从「荧光琥珀胶囊 + 💾 emoji」改为「细边框 ghost 按钮 + 书签 SVG」（与下载 MP3 同语言，hover 描边变琥珀）；`.sec-head` 全站加白底圆角卡片样式让标题脱离背景图装饰区（成品/情绪曲线/我的书架 三处一致）；「我的书架」去 🌟 emoji 换 16px 书堆 SVG（accent-deep 描边色）；行内「↻ 刷新」升级为独立 ghost 按钮推到 sec-actions 右侧。
19. **全站 emoji → 细线 SVG 清扫**（commit 52b90f4 后推，已部署）：两页结构性 emoji 全量替换为 1.7px stroke SVG（stroke=currentColor）——工作台：🌙 睡前故事→月亮、书架空态 🔒/📚→锁/书堆 34px + 弱化副文案、⏳ 保存中→纯文本；睡前故事：书架 tab/历史/开场结尾/家长中心/说明行/播放页提示/clip pill 文字全部 SVG 化或纯文本。内容态 emoji（进度条 ⏳ 状态、随机封面）按产品语音保留。grep 验证两页结构性 emoji 清零。
20. **书架说明行布局修复**（commit 后推，已部署）：说明行 SVG 无尺寸约束（14px 规则只挂在 .tb 下）被撑成巨图，文字列被挤成三行带孤字"听"，且说明行重复了两条。重建为专用 .shelf-note 块：15px 图标（accent-deep 75% 透明度）+ 单行文字（<560px 断行）+ 文案微调"来自工作台的成品有声书 · 点击即可收听"。
21. **产品定位调整：PRD/README/宪章 三文档同步**（PRD/README commit bc7c6f3，宪章随本次，纯文档无需部署）：定位改为「有声读物工作台 + 睡前故事播放器」双核心，目标用户收窄为亲子家庭。PRD 升 v2.0——新增双核心组合关系专段（工作台成品经书架流转到睡前故事页，生产消费闭环）、睡前故事播放器独立成章（P0 产品门面：六项核心能力 + 三组差异化对比）、工作台功能重排 F1-F7 并逐条标注与睡前场景的关系（音色三合一提为 P0）、里程碑按 S1/S2/S3 睡前故事线重排、系统边界修正（PG/账号体系移入"包含"）。README——标题/intro/新增产品定位段与 PRD 口径一致，功能特性重组为"睡前故事播放器（特色 P0）/工作台/通用基础"三组，目录结构补 story.html，数据库表补 user_listen_logs/user_audiobooks，删重复的"数据库"章节。PROJECT_CHARTER——项目名称与定位行更新，愿景收窄为亲子家庭两类角色（家长=生产+管控、孩子=消费），原三类人群注明"诉求重叠、Demo 不展开"。
