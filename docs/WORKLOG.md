...
10. **创作区排版优化：消除编辑器下方空闲大片留白**（前端 index.html 已上线）：
   - **根因**：textarea 固定 min-height 220px，空闲态进度/成品区全部隐藏 → 字数行与生成按钮下方出现大片死区（用户标注截图圈出）。
   - **方案**：编辑画布自适应视口——`height: clamp(260px, calc(100vh - 448px), 720px)`（448 = 上方头部/标题/页签 ≈273 + 下方字数行/按钮/底留白 ≈175），留白还给写作画布，字数/导入/生成按钮沉到首屏底部动线终点；260~720 兜底防极端屏，resize 手动拖拽仍可用。
   - **三分辨率实测**（1500×950 / 1366×728 / 1690×1130）：生成按钮均落首屏内（btnBottom 893/671/1073 ≤ vh），无死区。commit c80201c 推送。
