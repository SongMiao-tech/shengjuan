...
- **家长开场 / 结尾**：家长写一句开场语与晚安语（各 ≤100 字）→ 用音色库所选音色实时合成 → 开启后按「开场语 → 故事 → 结尾语」连续播放，未合成片段自动跳过；编辑页内可随时更换音色（与「爸妈的声音讲」同源音色库，记住上次选择）
- **收听历史**：「历史」页只显示收听记录，按故事聚合（最近日期 · 累计分钟 · 次数 · 听完徽章），点击直接回放；登录云端同步，断网回落本机记录
